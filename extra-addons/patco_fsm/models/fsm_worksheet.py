...
class FSMWorksheet(models.Model):
    ...
    technician_id = fields.Many2one(
        'res.partner',
        string='Técnico',
        related='order_id.person_id.partner_id',
        store=True,
        readonly=True
    )
    ...
    # Campo computado para porcentaje de completitud
    completion_percentage = fields.Float(
        string='Porcentaje de Completitud',
        compute='_compute_completion_percentage',
        store=True
    )
    
    @api.depends('state', 'technician_signature', 'customer_signature')
    def _compute_completion_percentage(self):
        """Calcula el porcentaje de completitud de la hoja de trabajo"""
        for record in self:
            if record.state == 'draft':
                ...
                if record.technician_signature and record.customer_signature:
                    record.completion_percentage = 100.0
                elif record.technician_signature:
    ...
    def action_generate_pdf(self):
        ...
        report = self.env.ref('patco_fsm.action_report_fsm_worksheet')
    ...
    def _generate_signed_pdf(self):
        ...
        report = self.env.ref('patco_fsm.action_report_fsm_worksheet_signed')
        pdf_content, _ = report._render_qweb_pdf([self.id])
